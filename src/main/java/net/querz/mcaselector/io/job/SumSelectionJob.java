...
public final class SumSelectionJob extends ProcessDataJob {
  
	private static final Logger LOGGER = LogManager.getLogger(SumSelectionJob.class);
	...
	public static AtomicLong sumSelection(Selection data, Overlay overlay, Progress progressChannel) {
		...
		for (Long2ObjectMap.Entry<ChunkSet> entry : data) {
			SumSelectionJob job = new SumSelectionJob(new Point2i(entry.getLongKey()), entry.getValue(), answer, overlay, progressChannel);
	...
	public SumSelectionJob(Point2i region, ChunkSet chunks, AtomicLong sum, Overlay parser, Progress progressChannel) {
