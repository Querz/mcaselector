...
public final class SelectFilterJob extends ProcessDataJob {

	private static final Logger LOGGER = LogManager.getLogger(SelectFilterJob.class);
	...
	public static void selectFilter(GroupFilter filter, Selection selection, int radius, Consumer<Selection> callback, Progress progressChannel, boolean cli) {
		...
		for (RegionDirectories r : rd) {
			SelectFilterJob job = new SelectFilterJob(r, filter, selection, callback, radius, progressChannel);
	...
	private SelectFilterJob(RegionDirectories dirs, GroupFilter filter, Selection selection, Consumer<Selection> callback, int radius,  Progress progressChannel) {
