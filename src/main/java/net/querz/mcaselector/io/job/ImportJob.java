...
public final class ImportJob extends ProcessDataJob {
	...
	public static void importChunks(WorldDirectories source, Progress progressChannel, boolean cli, boolean overwrite, Selection sourceSelection, Selection targetSelection, List<Range> ranges, Point3i offset, DataProperty<Map<Point2i, RegionDirectories>> tempFiles) {
		try {
			...
			for (Long2ObjectMap.Entry<LongSet> entry : targetMapping.long2ObjectEntrySet()) {
				...
				ImportJob job = new ImportJob(targetDirs, source, target, entry.getValue(), offset, progressChannel, overwrite, sourceSelection, targetSelection, ranges, tempFilesMap);
	...
	private ImportJob(RegionDirectories targetDirs, WorldDirectories sourceDirs, Point2i target, LongSet sourceRegions, Point3i offset, Progress progressChannel, boolean overwrite, Selection sourceSelection, Selection targetSelection, List<Range> ranges, Map<Point2i, RegionDirectories> tempFilesMap) {
	...
	public boolean execute() {
		...
		try {
			...
			targetRegion.saveWithTempFiles();
