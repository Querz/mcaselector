...
public final class ExportImageSelectionJob extends ProcessDataJob {

	private static final Logger LOGGER = LogManager.getLogger(ExportImageSelectionJob.class);
	...
	public static int[] exportSelectionImage(SelectionData data, OverlayPool overlayPool, Progress progressChannel) {
		...
		for (Long2ObjectMap.Entry<ChunkSet> entry : data.getSelection()) {
			ExportImageSelectionJob job = new ExportImageSelectionJob(new Point2i(entry.getLongKey()), entry.getValue(), data, pixels, overlayPool, progressChannel);
	...
	public ExportImageSelectionJob(Point2i region, ChunkSet chunks, SelectionData data, int[] pixels, OverlayPool overlayPool, Progress progressChannel) {
	...
	public boolean execute() {
		...
		Selection.regionIterator(chunks, getRegionDirectories().getLocation()).forEachRemaining(chunk -> {
