...
public class Anvil121ColorMapping implements ColorMapping {

	private static final Logger LOGGER = LogManager.getLogger(Anvil121ColorMapping.class);
	...
	public Anvil121ColorMapping() {
		// note_block:pitch=1,powered=true,instrument=flute;01ab9f
		try (BufferedReader bis = new BufferedReader(
			new InputStreamReader(Objects.requireNonNull(Anvil121ColorMapping.class.getClassLoader().getResourceAsStream("mapping/121/colors.txt"))))) {
			String line;
			...
			throw new RuntimeException("failed to read mapping/121/colors.txt");
		}

		try (BufferedReader bis = new BufferedReader(
			new InputStreamReader(Objects.requireNonNull(ColorMapping.class.getClassLoader().getResourceAsStream("mapping/121/biome_colors.txt"))))) {
			...
			throw new RuntimeException("failed to read mapping/121/biome_colors.txt");
		}
	}
