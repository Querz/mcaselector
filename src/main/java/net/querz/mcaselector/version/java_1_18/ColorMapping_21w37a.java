...
public class ColorMapping_21w37a implements ColorMapping {

	private static final Logger LOGGER = LogManager.getLogger(ColorMapping_21w37a.class);

	// value can either be an Integer (color) or a BlockStateMapping
	private static final Map<String, Object> mapping = new TreeMap<>();
	private static final Set<String> grass = new HashSet<>();
	private static final Set<String> foliage = new HashSet<>();

	private static final Map<String, Integer> biomeGrassTints = new HashMap<>();
	private static final Map<String, Integer> biomeFoliageTints = new HashMap<>();
	private static final Map<String, Integer> biomeWaterTints = new HashMap<>();

	static {
		// note_block:pitch=1,powered=true,instrument=flute;01ab9f
		try (BufferedReader bis = new BufferedReader(
				new InputStreamReader(Objects.requireNonNull(ColorMapping_21w37a.class.getClassLoader().getResourceAsStream("mapping/java_1_18/colors.txt"))))) {
			String line;
			...
			throw new RuntimeException("failed to read mapping/java_1_18/colors.txt");
		}

		try (BufferedReader bis = new BufferedReader(
				new InputStreamReader(Objects.requireNonNull(ColorMapping.class.getClassLoader().getResourceAsStream("mapping/java_1_18/biome_colors.txt"))))) {
			...
			throw new RuntimeException("failed to read mapping/java_1_18/biome_colors.txt");
		}
	}

	@Override
	public int getRGB(Object o, int biome) {
		throw new UnsupportedOperationException("color mapping for 1.18 does not support biome ids");
	}
