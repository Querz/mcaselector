...
public class ChunkRenderer_26_3_snapshot_7 implements ChunkRenderer<Tag, String> {

	@Override
	public void drawChunk(CompoundTag root, ColorMapping<Tag, String> colorMapping, int x, int z, int scale, int[] pixelBuffer, int[] waterPixels, short[] terrainHeights, short[] waterHeights, boolean water, int height) {
			...
			for (int cz = 0; cz < 16; cz += scale) {
				...
				for (int i = sMax - (sMax - (absHeight >> 4)); i >= 0; i--) {
					...
					for (int cy = startHeight[i]; cy >= 0; cy--) {
						Tag blockData = getBlock(cx, cy, cz, blockStates, bits[i], cleanBits[i], indexesPerLong[i], palette);
	...
	@Override
	public void drawLayer(CompoundTag root, ColorMapping<Tag, String> colorMapping, int x, int z, int scale, int[] pixelBuffer, int height) {
		ListTag sections = Helper.tagFromCompound(root, "sections");
		if (sections == null) {
			return;
		}
		...
		for (int cx = 0; cx < 16; cx += scale) {
			for (int cz = 0; cz < 16; cz += scale) {
				Tag blockData = getBlock(cx, cy, cz, blockStates, bits, clean, indexesPerLong, palette);
				if (colorMapping.isTransparent(blockData)) {
					continue;
				}
				pixelIndex = (z + (cz >> scaleBits)) * (512 >> scaleBits) + (x + (cx >> scaleBits));
	...
	@Override
	public void drawCaves(CompoundTag root, ColorMapping<Tag, String> colorMapping, int x, int z, int scale, int[] pixelBuffer, short[] terrainHeights, int height) {
		ListTag sections = Helper.tagFromCompound(root, "sections");
		if (sections == null || sections.getElementType() != Tag.Type.COMPOUND) {
			return;
		}
	...
	static {
		waterDummy.putString("", "minecraft:water");
	}
	...
	private Tag getBlock(int x, int y, int z, LongBuffer blockStates, int bits, int clean, int indexesPerLong, ListTag palette) {
		if (bits == 0) {
		...
		int startBit = (index % indexesPerLong) * bits;
		return palette.get((int) (blockStates.get(blockStatesIndex) >> startBit) & clean);
	}
}
