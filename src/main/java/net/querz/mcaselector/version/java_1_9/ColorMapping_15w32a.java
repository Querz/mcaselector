...
public class ColorMapping_15w32a implements ColorMapping<Integer, Integer> {
	...
	private static final Map<Integer, Integer> mapping = FileHelper.loadFromResource("mapping/java_1_9/colors.csv", p -> {
	...
	static {
		...
		FileHelper.loadFromResource("mapping/java_1_9/biome_colors.csv", p -> {
			try (Stream<String> lines = Files.lines(p)) {
				lines.forEach(line -> {
					String[] split = line.split(";");
					int id = Integer.parseInt(split[0]);
