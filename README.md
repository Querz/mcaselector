...
[**Download Version 1.16 (Windows Installer)**](https://github.com/Querz/mcaselector/releases/download/1.16/MCA_Selector_Setup.exe)
...
[**Download Version 1.16 (Universal)**](https://github.com/Querz/mcaselector/releases/download/1.16/mcaselector-1.16.jar)
...
* Download the JRE-FX-16 from [HERE](https://cdn.azul.com/zulu/bin/zulu16.30.15-ca-fx-jre16.0.1-win_x64.zip) into an 
  empty folder.
* Move the previously downloaded `mcaselector-1.16.jar` into the same folder. Extract the content of the 
...
* Hold `Shift` and `right-click` an empty spot in this folder, and choose `Open PowerShell window here`. Type `& ` 
  ...
  the PowerShell window. Then complete the command by typing a space and ` -jar mcaselector-1.16.jar` and press 
  `Enter`.
...
* Press `Cmd+Space`, type `Terminal` and press `Enter`. Go into the previously extracted folder and darg and drop 
  ...
  drag-and drop the `mcaselector-1.16.jar` into the terminal as well, the press `Enter`.
...
"When I run `mcaselector-1.16.jar`, an error dialog appears that looks like this:"
...
* Type `java -jar ` (with a space at the end) and drag and drop the `mcaselector-1.16.jar` into the console and 
  hit `Enter`.

For Linux:
* Run `java -jar <path to mcaselector-1.16.jar` where you replace everything in `<>`.
...
* Hold `Shift` and Right-click on an empty space in that folder and select `Open PowerShell window here` (`Open 
  ...
  at the beginning and the end), then drag and drop the `mcaselector-1.16.jar` into the console and hit `Enter`.
...
* Press `Cmd+Space`, type `Terminal` and press `Enter`. Type `java --module-path ` (with a space at the end), then 
  ...
  space at the end), then drag and drop the `mcaselector-1.16.jar` into the console and hit `Enter`.
...
* Open the command prompt and run `java --module-path <path to unzipped folder>/lib --add-modules ALL-MODULE-PATH 
  -jar <path to mcaselector-1.16.jar>` where you replace everything in `<>` with the appropriate paths.
* Some distributions like AdoptOpenJDK (shipped with most Linux distributions) do not ship with JavaFX by default. 
  ...
  JavaFX is required to run the `mcaselector-1.16.jar`.
